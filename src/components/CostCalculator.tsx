...
const COPY = {
  ko: {
    ...
    inputHint: '실시간 오디오 워크플로에 연결된 동안에는 100%로 두세요. 낮은 값은 전체 앱 사용 시간 중 유료 음성 엔진을 켠 비율을 가정할 때만 사용하며, OpenAI translation 세션 안의 침묵을 잘라낸다는 뜻이 아닙니다.',
    output: '번역 음성 길이',
    ...
    names: {
      'text-first-browser': '브라우저 기기 내 워크플로',
      'text-first-gemini': '데스크탑/모바일 · Gemini 텍스트 번역',
    ...
    notes: {
      ...
      'text-first-gemini': '브라우저 STT 또는 모바일 키보드 받아쓰기와 기본 TTS는 $0이고 Gemini 3.5 Flash-Lite 텍스트 번역만 추정합니다.',
  ...
  en: {
    ...
    inputHint: 'Use 100% while a realtime audio workflow is connected. Use a lower value only to model the share of total app time when a paid audio engine is on; it does not mean trimming silence inside an OpenAI translation session.',
    ...
    names: {
      'text-first-browser': 'Browser on-device workflow',
      'text-first-gemini': 'Desktop/mobile · Gemini text translation',
    ...
    notes: {
      ...
      'text-first-gemini': 'Browser STT or mobile keyboard dictation and platform TTS are $0; only Gemini 3.5 Flash-Lite text translation is estimated.',
  ...
  ja: {
    ...
    hoursUnit: '時間',
    input: '有料音声エンジン利用率',
    inputHint: 'リアルタイム音声ワークフローへの接続中は100%にしてください。低い値は、アプリの総利用時間のうち有料音声エンジンを有効にする割合を試算する場合だけに使います。OpenAI translationセッション内の無音を切り取るという意味ではありません。',
    output: '翻訳音声の長さ',
    ...
    names: {
      'text-first-browser': 'ブラウザ内デバイスワークフロー',
      'text-first-gemini': 'デスクトップ/モバイル · Geminiテキスト翻訳',
    ...
    notes: {
      ...
      'text-first-gemini': 'ブラウザSTTまたはモバイルキーボード音声入力と端末標準TTSは$0で、Gemini 3.5 Flash-Liteテキスト翻訳のみを推定します。',
  ...
  'zh-TW': {
    ...
    hoursUnit: '小時',
    input: '付費語音引擎使用比例',
    inputHint: '連線至即時音訊工作流程期間請設為 100%。較低數值只用於估算整體應用程式使用時間中啟用付費語音引擎的比例；並不表示裁切 OpenAI translation 工作階段內的靜音。',
    output: '翻譯音訊長度',
    ...
    names: {
      'text-first-browser': '瀏覽器裝置端工作流程',
      'text-first-gemini': '桌面/行動 · Gemini 文字翻譯',
    ...
    notes: {
      ...
      'text-first-gemini': '瀏覽器 STT 或行動鍵盤語音輸入與裝置預設 TTS 為 $0，只估算 Gemini 3.5 Flash-Lite 文字翻譯。',
  ...
  zh: {
    ...
    hoursUnit: '小时',
    input: '付费语音引擎使用比例',
    inputHint: '连接到实时音频工作流程期间请设为 100%。较低数值仅用于估算整个应用使用时间中启用付费语音引擎的比例；并不表示裁切 OpenAI translation 会话内的静音。',
    output: '翻译音频时长',
    ...
    names: {
      'text-first-browser': '浏览器设备端工作流程',
      'text-first-gemini': '桌面/移动 · Gemini 文本翻译',
    ...
    notes: {
      ...
      'text-first-gemini': '浏览器 STT 或移动键盘语音输入和设备默认 TTS 为 $0，只估算 Gemini 3.5 Flash-Lite 文本翻译。',
  ...
  es: {
    ...
    inputHint: 'Usa el 100% mientras esté conectado un flujo de audio en tiempo real. Usa un valor inferior solo para representar la proporción del tiempo total de la aplicación durante la que un motor de voz de pago está activo; no significa recortar el silencio dentro de una sesión de OpenAI translation.',
    ...
    names: {
      'text-first-browser': 'Flujo en el dispositivo del navegador',
      'text-first-gemini': 'Escritorio/móvil · traducción de texto Gemini',
    ...
    notes: {
      ...
      'text-first-gemini': 'El STT del navegador o el dictado del teclado móvil y el TTS de la plataforma cuestan $0; solo se estima la traducción de texto de Gemini 3.5 Flash-Lite.',
  ...
  fr: {
    ...
    inputHint: 'Utilisez 100 % tant qu’un flux audio en temps réel est connecté. Une valeur inférieure sert uniquement à représenter la part du temps total d’utilisation pendant laquelle un moteur vocal payant est actif ; elle ne signifie pas que le silence est retiré au sein d’une session OpenAI translation.',
    ...
    names: {
      'text-first-browser': 'Flux local du navigateur',
      'text-first-gemini': 'Ordinateur/mobile · traduction texte Gemini',
    ...
    notes: {
      ...
      'text-first-gemini': 'Le STT du navigateur ou la dictée clavier mobile et le TTS de la plateforme coûtent $0 ; seule la traduction texte Gemini 3.5 Flash-Lite est estimée.',
  ...
  de: {
    ...
    inputHint: 'Während ein Echtzeit-Audioablauf verbunden ist, verwenden Sie 100 %. Ein niedrigerer Wert dient nur dazu, den Anteil der gesamten App-Nutzungszeit mit aktiver kostenpflichtiger Audio-Engine abzubilden; er bedeutet nicht, dass Stille innerhalb einer OpenAI-translation-Sitzung herausgeschnitten wird.',
    ...
    names: {
      'text-first-browser': 'Browser-interner Geräteablauf',
      'text-first-gemini': 'Desktop/Mobil · Gemini-Textübersetzung',
    ...
    notes: {
      ...
      'text-first-gemini': 'Browser-STT oder mobiles Tastaturdiktat und Plattform-TTS kosten $0; nur die Gemini-3.5-Flash-Lite-Textübersetzung wird geschätzt.',
  ...
  vi: {
    ...
    inputHint: 'Đặt 100% trong khi quy trình âm thanh thời gian thực đang kết nối. Chỉ dùng giá trị thấp hơn để mô phỏng tỷ lệ trong tổng thời gian dùng ứng dụng mà công cụ giọng nói trả phí được bật; điều này không có nghĩa là cắt khoảng lặng bên trong phiên OpenAI translation.',
    ...
    names: {
      'text-first-browser': 'Quy trình trên thiết bị của trình duyệt',
      'text-first-gemini': 'Máy tính/di động · dịch văn bản Gemini',
    ...
    notes: {
      ...
      'text-first-gemini': 'STT trình duyệt hoặc nhập giọng nói bằng bàn phím di động và TTS nền tảng là $0; chỉ ước tính dịch văn bản bằng Gemini 3.5 Flash-Lite.',
