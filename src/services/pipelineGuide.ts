...
const GUIDE_COPY: Record<GuideLanguage, GuideCopy> = {
  en: {
    engines: {
      ...
      turbo_fastpath: {
        ...
        speed: 'Latency can vary widely with the external service connection and response time.',
        accuracy: 'Suitable for simple sentences but less reliable for long context and idiomatic language.',
  ...
  ko: {
    engines: {
      auto: {
        ...
        privacy: 'Chrome 번역기를 사용할 수 있으면 기기에서 번역합니다. 그렇지 않으면 전사문이 Gemini 또는 외부 번역 서비스로 전송될 수 있습니다.',
        requirements: 'Web Speech와 브라우저 TTS가 필요합니다. Chrome 번역기, API 키를 사용하는 Gemini, 네트워크 대체 경로 중 사용 가능한 경로를 자동으로 찾습니다.',
        caution: '속도, 품질, 데이터 처리 방식은 실행 중 선택된 엔진에 따라 달라집니다. 현재 엔진 표시를 확인하세요.',
      },
      chrome_nano: {
        situation: '지원되는 데스크톱 Chrome에서 개인정보 보호와 반복 연습의 짧은 지연 시간이 가장 중요할 때 적합합니다.',
        ...
        requirements: '내장 Translator API를 지원하는 데스크톱 Chrome과 선택한 언어 조합용으로 내려받을 수 있는 모델이 필요합니다.',
      ...
      turbo_fastpath: {
        situation: 'Chrome 번역기를 사용할 수 없고 Gemini API 키도 없을 때 쓰는 임시 호환 경로입니다.',
        speed: '외부 서비스의 연결과 응답 시간에 따라 지연 시간이 크게 달라질 수 있습니다.',
        accuracy: '간단한 문장에는 적합하지만 긴 문맥과 관용 표현에서는 신뢰도가 낮습니다.',
        privacy: '전사문이 Google Translate나 MyMemory 같은 외부 서비스로 전송되며 오프라인에서는 작동하지 않습니다.',
        requirements: 'API 키는 필요 없지만 브라우저가 외부 번역 서비스에 접속할 수 있어야 합니다.',
        caution: '이 대체 경로는 가용성이 보장되지 않아 속도와 품질이 달라질 수 있습니다. 민감한 내용에는 사용하지 마세요.',
      },
    },
    profiles: {
    ...
    outputSpeed: {
      ...
      auto: '자동 선택이 Gemini 스트리밍을 고르면 완성된 구절을 일찍 재생합니다. Chrome과 네트워크 엔진은 표준 출력처럼 전체 결과 뒤에 재생합니다.',
  ...
  ja: {
    engines: {
      ...
      turbo_fastpath: {
        situation: 'Chrome 翻訳が利用できず、Gemini API キーもない場合の一時的な互換経路です。',
        speed: '外部サービスへの接続状態と応答時間により、遅延が大きく変わることがあります。',
        accuracy: '簡単な文には使えますが、長い文脈や慣用表現では信頼性が下がります。',
        privacy: '文字起こしは Google Translate や MyMemory などの外部サービスに送信され、オフラインでは動作しません。',
        requirements: 'API キーは不要ですが、ブラウザから外部翻訳サービスへ接続できる必要があります。',
        caution: 'この代替経路は可用性が保証されず、速度と品質が変動します。機密性の高い内容には使用しないでください。',
      },
    },
    profiles: {
  ...
  'zh-TW': {
    engines: {
      ...
      turbo_fastpath: {
        situation: '無法使用 Chrome 翻譯工具且沒有 Gemini API 金鑰時的暫時相容路徑。',
        speed: '延遲可能因外部服務的連線與回應時間而大幅變動。',
        accuracy: '適合簡單句子，但對較長語境和慣用語的可靠度較低。',
        privacy: '逐字稿會傳送至 Google Translate 或 MyMemory 等外部服務，且無法離線運作。',
        requirements: '不需要 API 金鑰，但瀏覽器必須能連上外部翻譯服務。',
        caution: '此備援路徑不保證可用，因此速度與品質可能變動。請勿用於敏感內容。',
      },
    },
    profiles: {
  ...
  zh: {
    engines: {
      ...
      turbo_fastpath: {
        situation: '无法使用 Chrome 翻译器且没有 Gemini API 密钥时的临时兼容路径。',
        speed: '延迟可能会随外部服务的连接和响应时间而大幅变化。',
        accuracy: '适合简单句子，但对较长语境和习语的可靠性较低。',
        privacy: '转写文本会发送到 Google Translate 或 MyMemory 等外部服务，且无法离线工作。',
        requirements: '无需 API 密钥，但浏览器必须能够连接外部翻译服务。',
        caution: '此备用方案不保证可用，因此速度和质量可能会有变化。请勿用于敏感内容。',
      },
    },
    profiles: {
  ...
  es: {
    engines: {
      ...
      turbo_fastpath: {
        situation: 'Es una ruta de compatibilidad temporal cuando el traductor de Chrome no está disponible y no hay una clave de API de Gemini.',
        speed: 'La latencia puede variar mucho según la conexión y el tiempo de respuesta del servicio externo.',
        accuracy: 'Es adecuada para frases sencillas, pero menos fiable con contextos largos y expresiones idiomáticas.',
        privacy: 'La transcripción se envía a un servicio externo como Google Translate o MyMemory y no funciona sin conexión.',
        requirements: 'No requiere clave de API, pero el navegador debe poder acceder a un servicio de traducción externo.',
        caution: 'Esta alternativa no garantiza su disponibilidad, por lo que la velocidad y la calidad pueden variar. No la uses con contenido confidencial.',
      },
    },
    profiles: {
  ...
  fr: {
    engines: {
      ...
      turbo_fastpath: {
        situation: 'Voie de compatibilité temporaire lorsque le traducteur Chrome est indisponible et qu’aucune clé API Gemini n’est fournie.',
        speed: 'La latence peut varier fortement selon la connexion au service externe et son temps de réponse.',
        accuracy: 'Adapté aux phrases simples, mais moins fiable pour les contextes longs et les expressions idiomatiques.',
        privacy: 'La transcription est envoyée à un service externe comme Google Translate ou MyMemory et ne fonctionne pas hors ligne.',
        requirements: 'Aucune clé API n’est requise, mais le navigateur doit pouvoir joindre un service de traduction externe.',
        caution: 'La disponibilité de cette solution de repli n’est pas garantie ; vitesse et qualité peuvent varier. Ne l’utilisez pas pour des contenus sensibles.',
      },
    },
    profiles: {
  ...
  de: {
    engines: {
      ...
      turbo_fastpath: {
        situation: 'Ein vorübergehender Kompatibilitätsweg, wenn der Chrome-Übersetzer nicht verfügbar und kein Gemini-API-Schlüssel vorhanden ist.',
        speed: 'Die Latenz kann je nach Verbindung und Antwortzeit des externen Dienstes stark schwanken.',
        accuracy: 'Für einfache Sätze geeignet, bei langem Kontext und Redewendungen jedoch weniger zuverlässig.',
        privacy: 'Das Transkript wird an einen externen Dienst wie Google Translate oder MyMemory gesendet und funktioniert nicht offline.',
        requirements: 'Kein API-Schlüssel erforderlich, der Browser muss jedoch einen externen Übersetzungsdienst erreichen können.',
        caution: 'Die Verfügbarkeit dieses Fallbacks ist nicht garantiert; Geschwindigkeit und Qualität können schwanken. Nicht für vertrauliche Inhalte verwenden.',
      },
    },
    profiles: {
  ...
  vi: {
    engines: {
      ...
      turbo_fastpath: {
        situation: 'Đường tương thích tạm thời khi Trình dịch Chrome không khả dụng và không có khóa Gemini API.',
        speed: 'Độ trễ có thể thay đổi nhiều tùy theo kết nối và thời gian phản hồi của dịch vụ bên ngoài.',
        accuracy: 'Phù hợp với câu đơn giản nhưng kém tin cậy hơn với ngữ cảnh dài và thành ngữ.',
        privacy: 'Bản chép lời được gửi đến dịch vụ bên ngoài như Google Translate hoặc MyMemory và không hoạt động ngoại tuyến.',
        requirements: 'Không cần khóa API nhưng trình duyệt phải kết nối được với dịch vụ dịch bên ngoài.',
        caution: 'Phương án dự phòng này không được bảo đảm luôn khả dụng nên tốc độ và chất lượng có thể thay đổi. Không dùng cho nội dung nhạy cảm.',
      },
    },
    profiles: {
